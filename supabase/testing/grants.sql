...
-- Real Supabase grants full CRUD (not just select) to both authenticated
-- AND anon by default — RLS is the only real gate, which is exactly why
-- e.g. complete_sandbox_deposit needed an explicit `revoke ... from anon`
-- in its own migration rather than relying on a narrower baseline grant.
