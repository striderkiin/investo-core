  ...
  return (
    <>
      <section className="position-relative overflow-hidden">
        <div className="ic-public-grid-lines" />
        <div className="ic-public-glow" style={{ width: 560, height: 560, top: -180, left: '20%', background: 'var(--pub-glow)' }} />
        <div className="ic-public-glow" style={{ width: 420, height: 420, top: 40, right: '10%', background: '#7c3aed' }} />
        <div className="container py-5 text-center position-relative">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <h1 className="display-4 fw-bold mb-3 mx-auto" style={{ maxWidth: 780 }}>
              Invest with <span className="ic-public-gradient-text">confidence</span>, in real time
            </h1>
            ...
            {STATS.map((stat) => (
              <motion.div className="col-4" key={stat.label} variants={staggerItem}>
                <div className="ic-public-card p-3">
                  <div className="h3 fw-bold mb-0 ic-public-gradient-text">
                    {stat.value}
                    {stat.suffix}
                  </div>
          ...
          {FEATURES.map((feature) => (
            <motion.div className="col-12 col-md-6 col-lg-3" key={feature.title} variants={staggerItem}>
              <div className="ic-public-card h-100 p-4 text-center">
                <i className={`bi ${feature.icon} fs-1 mb-3`} style={{ color: 'var(--pub-glow)' }} aria-hidden="true" />
                ...
                <motion.div className="col-12 col-md-6 col-lg-3" key={plan.id} variants={staggerItem}>
                  <div className={`ic-public-card h-100 p-4 d-flex flex-column ${index === plans.length - 1 ? 'ic-public-card-highlight' : ''}`}>
                    ...
                    <ul className="list-unstyled small mb-4 flex-grow-1">
                      <li className="mb-1">
                        <i className="bi bi-check2" style={{ color: 'var(--pub-glow)' }} aria-hidden="true" /> {formatCurrency(plan.minAmount)} –{' '}
                        {formatCurrency(plan.maxAmount)}
                      </li>
                      <li className="mb-1">
                        <i className="bi bi-check2" style={{ color: 'var(--pub-glow)' }} aria-hidden="true" /> {plan.durationDays}-day duration
                      </li>
                    </ul>
